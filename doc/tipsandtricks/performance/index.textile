...
# To cache into the file system is a good decision as long as the amount of data cached has a reasonable size. As the amount grows is will take longer and longer to write the data, while reading it remains fast. As a consequence you might loose as much or even more performance (when the cache is invalid) as you win, when the cache is valid.
# The file system is not that slow. But not writing to or reading from the disk will be much faster. If your server is equipped with APC you 