...
(function($){
	...
	$.fn.jsonSuggest = function(url, method, settings) {
		var defaults = {  
			minCharacters: 1,
		...
		return this.each(function() {
			...
			function buildResults(resultObjects, sFilterTxt) {
				...
				for (i = 0; i < resultObjects.length; i += 1) {
					...
					$(item).addClass('resultItem').
						addClass((bOddRow) ? 'aitsu_odd' : 'aitsu_even').
			...
			function getJson(e) {
				...
				$.getJSON(url, { method: method, searchfor: this.value }, function(json){searchData = json; runSuggest.apply(currentObject, [e]);});
			}
