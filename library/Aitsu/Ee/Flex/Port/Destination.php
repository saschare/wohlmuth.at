...
class Aitsu_Ee_Flex_Port_Destination {
	...
	public static function getInstance($idartlang, $destinationName = null) {
		...
		if (!isset ($instance)) {
			if ($destinationName == null) {
				throw new Aitsu_Ee_Flex_Port_Exception_DestinationNameMissing('' .
	...
	public function addArticle($idartlang, $template, $position, $from = null, $until = null) {
		...
		Aitsu_Ee_Flex_Port :: getInstance()->addArticle($idartlang, $template, $this, $position, $from, $until);
	}

	public function removeArticle($idartlang) {

		Aitsu_Ee_Flex_Port :: getInstance()->removeArticle($idartlang, $this);
	}
	...
	public function getOut($edit = false) {
		 ...
		 */
		Aitsu_Ee_Cache_Page :: lifetime(15 * 60);

		/*
		 * Then the persistence has to be loaded.
		 */
		$p = Aitsu_Ee_Persistence :: getInstance('FlexPortDestination', $this->idflexportdest);
	...
	protected function _buildOutput($edit) {
		...
		foreach ($results as $result) {
			...
			} catch (Aitsu_Ee_Flex_Port_Exception_RenderingNotPossible $e) {
				if ($edit) {
					$out[] = array (
						'template' => $result['template'],
						'output' => $e->getMessage(),
	...
	protected function _updatePositions() {
		...
		Aitsu_Ee_Flex_Port :: getInstance()->moveSourceInFrontOfTarget($source[0], $target[0], $_POST['flexPortDest']);
	}
}