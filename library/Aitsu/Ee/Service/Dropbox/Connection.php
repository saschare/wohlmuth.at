...
class Aitsu_Ee_Service_Dropbox_Connection {
	...
	public static function getInstance($email = null, $password = null) {
		
		static $instance;
		
		if (!isset($instance) || ($email != null && $password != null)) {
	...
	public function upload($filename, $remoteDir = '/') {
		...
		$data = $this->request('https://dl-web.dropbox.com/upload', true, array (
			'plain' => 'yes',
			'file' => '@' . $filename,
			'dest' => $remoteDir,
			't' => $token
	...
	public function getfiles($remoteDir = '/') {
		...
		$file_names = array();
		preg_match_all('|<div.*?details-filename.*?href=.*/([^/]*)\\?w=(.*)\"|', $data, $file_array);
		
		for ($i = 0; $i < count($file_array[0]); $i++) {
			$file_names[] = array($file_array[1][$i], $file_array[2][$i]);
		}
	...
	protected function login() {
		...
		$data = $this->request('https://www.dropbox.com/login', true, array (
