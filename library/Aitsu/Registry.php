...
class Aitsu_Registry {
    ...
    public static function setExpireTime($time) {
		...
		if (!isset($reg->registry['expireTime'])) {
			$reg->registry['expireTime'] = Aitsu_Config :: get('cache.browser.expireTime');
		}
	...
	public static function getExpireTime() {
		...
		if (!isset($reg->registry['expireTime'])) {
			$reg->registry['expireTime'] = isset($reg->config->cache->browser->expireTime) ? $reg->config->cache->browser->expireTime : 0;
		}
