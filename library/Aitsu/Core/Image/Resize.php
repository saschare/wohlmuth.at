...
class Aitsu_Core_Image_Resize {
	...
	protected function _makeAvailableTransparent($imagename) {
		...
		if (preg_match('@^\\d*\\/\\d*\\/\\d*\\/(\\d*)@', $pathInfo['dirname'], $match)) {
			if (Aitsu_Db :: fetchOne('' .
				'select min(catlang.public) ' .
				...
				/*
				 * The requested resource is in a protected area and should therefore
				 * not be saved within the transparent cache.
				 */
				return;
			}
		}
