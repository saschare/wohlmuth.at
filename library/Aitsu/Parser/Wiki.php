...
class Aitsu_Parser_Wiki {
	
	protected $reference_wiki = '';
	protected $image_uri = '';
	protected $ignore_images = true;
	...
	protected function handle_image($href, $title, $options) {
		...
		foreach ($options as $k => $option) {
			switch ($option) {
				case 'frame' :
					$imagetag = sprintf('<div style="float: right; background-color: #F5F5F5; border: 1px solid #D0D0D0; padding: 2px">' .
					'%s' .
					'<div>%s</div>' .
					'</div>', $imagetag, $title);
					break;
	...
	protected function handle_internallink($matches) {
		...
		return sprintf('<a href="%s"%s>%s</a>', $href, ($newwindow ? ' target="_blank"' : ''), $title);
	}
	...
	')',
		...
		if (!$this->stop_all) {
			...
			foreach ($char_regexes as $func => $regex) {
				$line = preg_replace_callback("/$regex/i", array (
					& $this,
					"handle_" . $func
				), $line);
				if ($this->stop)
	...
	public static function parse($text) {
		...
		$instance->nowikis = array ();
		$instance->list_level_types = array ();
		$instance->list_level = 0;

		$instance->deflist = false;
		$instance->linknumber = 0;
		$instance->suppress_linebreaks = false;

		$instance->page_title = '';

		$output = "";

		$text = preg_replace_callback('/<nowiki>([\s\S]*)<\/nowiki>/i', array (
			& $instance,
			"handle_save_nowiki"
		), $text);
		...
		if (preg_match('/^\#REDIRECT\s+\[\[(.*?)\]\]$/', trim($lines[0]), $matches)) {
			$instance->redirect = $matches[1];
		}

		foreach ($lines as $k => $line) {
			$line = $instance->parse_line($line);
			$output .= $line;
		}

		$output = preg_replace_callback('/<nowiki><\/nowiki>/i', array (
			& $instance,
			"handle_restore_nowiki"
		), $output);
